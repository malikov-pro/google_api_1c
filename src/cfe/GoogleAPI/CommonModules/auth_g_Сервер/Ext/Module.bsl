...
Функция ПолучитьТокен(СервисныйПользователь, ПраваДоступа) Экспорт
	...
	|	auth_g_Токены.Токен КАК Токен
	|ИЗ
	|	РегистрСведений.auth_g_Токены КАК auth_g_Токены
	|ГДЕ
	|	auth_g_Токены.СервисныйПользователь = &СервисныйПользователь
	|	И auth_g_Токены.ДатаИстечения < &ДатаИстечения
	|	И auth_g_Токены.ПраваДоступа ПОДОБНО &ПраваДоступа";
...
Функция ЗаписатьТокен(СервисныйПользователь, СоответствиеОтвет)
	...
	МЗ = РегистрыСведений.auth_g_Токены.СоздатьМенеджерЗаписи();
...
Функция ПолучитьJWT_RS256(client_email, scope, КлючПодписи_XML)
	...
	ЗаголовокЗапроса64 = auth_g_РаботаСоСтроками.Base64Url(ПолучитьДвоичныеДанныеИзСтроки(ЗаголовокЗапроса));
	
	ТелоЗапроса = ПолучитьТелоЗапроса(client_email, scope);
	ТелоЗапроса64 = auth_g_РаботаСоСтроками.Base64Url(ПолучитьДвоичныеДанныеИзСтроки(ТелоЗапроса));
	...
	Подпись64 = auth_g_РаботаСоСтроками.Base64Url(ПодписьДвоичная);
...
Функция ПолучитьЗаголовокЗапроса()
	...
	Возврат auth_g_РаботаСоСтроками.В_JSON(СтруктураЗаголовок);
...
Функция ПолучитьТелоЗапроса(client_email, scope)
	...
	Возврат auth_g_РаботаСоСтроками.В_JSON(СтруктураТелоЗапроса);
